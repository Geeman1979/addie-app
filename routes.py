...
def register_routes(app, db):
    ...
    # ── EXPORTS ──
    @app.route('/requests/export/<fmt>')
    @login_required
    def export_request_list(fmt):
        from openpyxl import Workbook
        from openpyxl.styles import Font as XlFont, PatternFill as XlFill
        ...
        if fmt == 'csv':
            output = io.StringIO()
            w = csv.writer(output)
            w.writerow(['ID','Title','Status','Priority','Phase','Type','Requester','Department','Assigned','Deadline','Created','ROI'])
            for r in reqs:
                w.writerow([r.id, r.title, r.status, r.priority, r.current_phase, r.request_type,
                    r.requester_name or '', r.requester_department or '', r.assigned_to.name if r.assigned_to else '',
                    str(r.deadline or ''), str(r.created_at.date()) if r.created_at else '', r.projected_roi or ''])
            resp = mkresp(output.getvalue())
            resp.headers['Content-Disposition'] = 'attachment; filename=interventions.csv'
            resp.headers['Content-type'] = 'text/csv'
            return resp
        wb = Workbook(); ws = wb.active; ws.title = 'Interventions'
        headers = ['ID','Title','Status','Priority','Phase','Type','Requester','Department','Assigned','Deadline','Created','Projected ROI']
        hf = XlFill(start_color='1B3A5C', end_color='1B3A5C', fill_type='solid')
        for c, h in enumerate(headers, 1):
            cell = ws.cell(row=1, column=c, value=h)
            cell.font = XlFont(bold=True, color='FFFFFF', size=10); cell.fill = hf
        for i, r in enumerate(reqs, 2):
            for c, v in enumerate([r.id, r.title, r.status, r.priority, r.current_phase, r.request_type,
                r.requester_name or '', r.requester_department or '', r.assigned_to.name if r.assigned_to else '',
                str(r.deadline or ''), str(r.created_at.date()) if r.created_at else '', r.projected_roi or ''], 1):
                ws.cell(row=i, column=c, value=v).font = XlFont(size=9)
        for i, w in enumerate([6,40,15,12,14,12,20,20,20,14,14,12], 1):
            ws.column_dimensions[chr(64+i)].width = w
        output = io.BytesIO(); wb.save(output); output.seek(0)
        resp = mkresp(output.getvalue())
        resp.headers['Content-Disposition'] = 'attachment; filename=interventions.xlsx'
        resp.headers['Content-type'] = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        return resp

    @app.route('/calendar/export/<fmt>')
    @login_required
    def export_calendar(fmt):
        from openpyxl import Workbook
        from openpyxl.styles import Font as XlFont, PatternFill as XlFill
        from flask import make_response as mkresp
        import csv, io
        ...
        if fmt == 'csv':
            output = io.StringIO()
            w = csv.writer(output)
            w.writerow(['Title','Event Type','Start Date','End Date','Status','Trainer','Location','Notes','Linked Request'])
            for e in events:
                w.writerow([e.title, e.event_type, str(e.start_date), str(e.end_date), e.status,
                    e.trainer.name if e.trainer else '', e.location or '', e.notes or '',
                    e.request.title if e.request else ''])
            resp = mkresp(output.getvalue())
            resp.headers['Content-Disposition'] = 'attachment; filename=calendar.csv'
            resp.headers['Content-type'] = 'text/csv'
            return resp
        wb = Workbook(); ws = wb.active; ws.title = 'Calendar Events'
        headers = ['Title','Event Type','Start Date','End Date','Status','Trainer','Location','Notes','Linked Request']
        hf = XlFill(start_color='1B3A5C', end_color='1B3A5C', fill_type='solid')
        for c, h in enumerate(headers, 1):
            cell = ws.cell(row=1, column=c, value=h)
            cell.font = XlFont(bold=True, color='FFFFFF', size=10); cell.fill = hf
        for i, e in enumerate(events, 2):
            for c, v in enumerate([e.title, e.event_type, str(e.start_date), str(e.end_date), e.status,
                e.trainer.name if e.trainer else '', e.location or '', e.notes or '',
                e.request.title if e.request else ''], 1):
                ws.cell(row=i, column=c, value=v).font = XlFont(size=9)
        for i, w in enumerate([40,18,14,14,14,20,20,30,40], 1):
            ws.column_dimensions[chr(64+i)].width = w
        output = io.BytesIO(); wb.save(output); output.seek(0)
        resp = mkresp(output.getvalue())
        resp.headers['Content-Disposition'] = 'attachment; filename=calendar.xlsx'
        resp.headers['Content-type'] = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        return resp

    @app.route('/audit-log/export/<fmt>')
    @login_required
    def export_audit_log(fmt):
        ...
        if fmt == 'csv':
            output = io.StringIO()
            w = csv.writer(output)
            w.writerow(['Date','User','Action','Entity','Entity ID','Details','IP Address'])
            for l in logs:
                w.writerow([str(l.created_at), l.user.name if l.user else '', l.action or '',
                    l.entity_type or '', l.entity_id or '', l.details or '', l.ip_address or ''])
            resp = mkresp(output.getvalue())
            resp.headers['Content-Disposition'] = 'attachment; filename=audit_log.csv'
            resp.headers['Content-type'] = 'text/csv'
            return resp
        wb = Workbook(); ws = wb.active; ws.title = 'Audit Log'
        headers = ['Date','User','Action','Entity','Entity ID','Details','IP Address']
        hf = XlFill(start_color='1B3A5C', end_color='1B3A5C', fill_type='solid')
        for c, h in enumerate(headers, 1):
            cell = ws.cell(row=1, column=c, value=h)
            cell.font = XlFont(bold=True, color='FFFFFF', size=10); cell.fill = hf
        for i, l in enumerate(logs, 2):
            for c, v in enumerate([str(l.created_at), l.user.name if l.user else '', l.action or '',
                l.entity_type or '', l.entity_id or '', l.details or '', l.ip_address or ''], 1):
                ws.cell(row=i, column=c, value=v).font = XlFont(size=9)
        for i, w in enumerate([22,20,18,14,10,40,16], 1):
            ws.column_dimensions[chr(64+i)].width = w
        output = io.BytesIO(); wb.save(output); output.seek(0)
        resp = mkresp(output.getvalue())
        resp.headers['Content-Disposition'] = 'attachment; filename=audit_log.xlsx'
        resp.headers['Content-type'] = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        return resp

    @app.route('/kanban/export/<fmt>')
    @login_required
    def export_kanban(fmt):
        from openpyxl import Workbook
        from openpyxl.styles import Font as XlFont, PatternFill as XlFill
        from flask import make_response as mkresp
        import csv, io
        ...
        if fmt == 'csv':
            output = io.StringIO()
            w = csv.writer(output)
            w.writerow(['Intervention','Phase','Resource Name','Role','Start Date','Inferred Completion','Notes'])
            for a in allocations:
                w.writerow([a.request.title if a.request else '', a.phase_name or '',
                    a.user.name if a.user else '', a.role or '',
                    str(a.start_date) if a.start_date else '', str(a.inferred_completion) if a.inferred_completion else '',
                    a.notes or ''])
            resp = mkresp(output.getvalue())
            resp.headers['Content-Disposition'] = 'attachment; filename=kanban.csv'
            resp.headers['Content-type'] = 'text/csv'
            return resp
        wb = Workbook(); ws = wb.active; ws.title = 'Kanban Resources'
        headers = ['Intervention','Phase','Resource Name','Role','Start Date','Inferred Completion','Notes']
        hf = XlFill(start_color='1B3A5C', end_color='1B3A5C', fill_type='solid')
        for c, h in enumerate(headers, 1):
            cell = ws.cell(row=1, column=c, value=h)
            cell.font = XlFont(bold=True, color='FFFFFF', size=10); cell.fill = hf
        for i, a in enumerate(allocations, 2):
            for c, v in enumerate([a.request.title if a.request else '', a.phase_name or '',
                a.user.name if a.user else '', a.role or '',
                str(a.start_date) if a.start_date else '', str(a.inferred_completion) if a.inferred_completion else '',
                a.notes or ''], 1):
                ws.cell(row=i, column=c, value=v).font = XlFont(size=9)
        for i, w in enumerate([40,14,20,16,14,16,30], 1):
            ws.column_dimensions[chr(64+i)].width = w

        # Gantt sheet — interventions with start/end dates from phase activities
        ws2 = wb.create_sheet('Gantt View')
        gheaders = ['Intervention','Task','Phase','Start Date','End Date','Duration (days)','Status','Assigned To']
        for c, h in enumerate(gheaders, 1):
            cell = ws2.cell(row=1, column=c, value=h)
            cell.font = XlFont(bold=True, color='FFFFFF', size=10); cell.fill = hf
        row_num = 2
        for req in Request.query.order_by(Request.title).all():
            for phase in req.phases.order_by(Phase.started_at).all():
                duration = (phase.completed_at - phase.started_at).days if phase.completed_at and phase.started_at else 0
                ws2.cell(row=row_num, column=1, value=req.title).font = XlFont(size=9)
                ws2.cell(row=row_num, column=2, value=f'{phase.phase_name.title()} phase').font = XlFont(size=9)
                ws2.cell(row=row_num, column=3, value=phase.phase_name.title()).font = XlFont(size=9)
                ws2.cell(row=row_num, column=4, value=str(phase.started_at.date()) if phase.started_at else '').font = XlFont(size=9)
                ws2.cell(row=row_num, column=5, value=str(phase.completed_at.date()) if phase.completed_at else '').font = XlFont(size=9)
                ws2.cell(row=row_num, column=6, value=duration).font = XlFont(size=9)
                ws2.cell(row=row_num, column=7, value=phase.status).font = XlFont(size=9)
                ws2.cell(row=row_num, column=8, value=req.assigned_to.name if req.assigned_to else '').font = XlFont(size=9)
                row_num += 1
        for i, w in enumerate([40,20,14,14,14,12,14,20], 1):
            ws2.column_dimensions[chr(64+i)].width = w
        output = io.BytesIO(); wb.save(output); output.seek(0)
        resp = mkresp(output.getvalue())
        resp.headers['Content-Disposition'] = 'attachment; filename=kanban_with_gantt.xlsx'
        resp.headers['Content-type'] = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        return resp
